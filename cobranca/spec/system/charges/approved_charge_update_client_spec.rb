...
describe 'User approval sends payment data to order API at Customer Panel' do
  it 'and an error is logged if a problem on API processing' do # rubocop:disable Metrics/BlockLength
    ...
    within('form#form1') do
      fill_in 'Código de Aprovação', with: approval_code
      click_on 'Aprovar Cobrança'
    end

    expect(page).to have_content 'A cobrança foi aprovada com sucesso'
    msg = "ERRO: Não foi possível atualizar os dados de pagamento do pedido #{cobranca1.order}"
  ...
  it 'and an error is logged if a problem to access API' do # rubocop:disable Metrics/BlockLength
    ...
    expect(page).to have_content 'A cobrança foi aprovada com sucesso'
    msg = "ERRO: Falha ao acessar #{url2} para gravar os dados de pagamento do pedido #{cobranca1.order}"
    expect(Rails.logger).to have_received(:error).with(msg)
  end
end
